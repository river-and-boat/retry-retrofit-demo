...
public class RetryCallAdapterFactory extends CallAdapter.Factory {
    ...
    private static RetryCallAdapterFactory factory;

    public static RetryCallAdapterFactory create() {
        return createFactory();
    }

    private static synchronized RetryCallAdapterFactory createFactory() {
        if (factory == null) {
            factory = new RetryCallAdapterFactory();
        }
        return factory;
    }
    ...
    public CallAdapter<?, ?> get(Type returnType, Annotation[] annotations, Retrofit retrofit) {
        ...
        if (retry != null) {
            retryCount = retry.max();
        }
        Log.d(TAG, "Starting a CallAdapter with {} retries." + retryCount);
            ...
            if (retryCount.incrementAndGet() <= maxRetries) {
                ...
                callback.onFailure(call,
                        new TimeoutException(
                                String.format("No retries left after %s attempts.", maxRetries)
                        )
                );
            } else {
